...
The same is sometimes true when making a test pass. For example, in [29120f5](https://github.com/matatk/NoughtsAndCrosses/commit/29120f5a4ac5a8ebe732c29dbfa1813df7dc0689) (below) the test has carelessly been made to pass outside of the test itself. In this case, we should have first added the extension `and ...` to the `is_legal` call and then use a refactoring to move the code into the production code.
...
MTA: This often got us into a very repetitive rut with the TDD process, where not much was added to the code, but we couldn’t seem to get off that path, as can be seen below.
{: style="color: blue" }
