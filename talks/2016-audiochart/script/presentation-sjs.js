...
function make_audio_chart(title, name1, t1, name2, rows, chart_id, trigger_id) {
	// Google charts bit
	var dataAndChart = make_chart(title, name1, t1, name2, rows, chart_id);

	// AudioChart Stuff
	var ac = new AudioChart({
		'type': 'google',             // (see the docs)
		'data': dataAndChart.data,    // the GoogleDataTable
		'chart': dataAndChart.chart,  // the Google Chart object
		'duration': 5000,             // milliseconds
		'frequencyLow': 200,          // Hz
		'frequencyHigh': 600          // Hz
	});

	document.getElementById(trigger_id).onclick = function() {
		ac.playPause();
	};
}
